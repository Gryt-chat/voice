...
import type { CaptureQuality, ScreenShareFps } from "../../types";
import { estimateBitrate } from "../lib/screenBitrate";
...
export { estimateBitrate };

/** Kept as a name because call sites use it. One list, in types.ts. */
export type ScreenShareQuality = CaptureQuality;

export type { ScreenShareFps };
...
function useScreenShareHook(): ScreenShareInterface {
  ...
  const startScreenShare = useCallback(async (withAudio: boolean, sourceId?: string) => {
    ...
    console.log(
      `[ScreenShare] startScreenShare withAudio=${withAudio} nativeAudio=${useNativeAudio} nativeVideo=${useNativeVideo} nativeCapture=${getVoiceHost().hasNativeCapture()} sourceId=${sourceId ?? "none"} fps=${fps} quality=${screenShareQuality}`,
    );
    ...
    try {
      ...
      if (getVoiceHost().hasNativeCapture() && sourceId) {
